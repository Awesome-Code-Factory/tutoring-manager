...
vi.mock("@/auth/user", () => ({ comparePasswords }));
vi.mock("@/auth/session", () => ({ createSession }));
vi.mock("@/db/user/select", () => ({ getUser }));
...
describe("login action", () => {
  describe("wrong form input", () => {
    ...
    it("should return error for password if only email passed", async () => {
      ...
      expect(result.errors.email).toHaveLength(0);
      expect(result.errors.password).not.toHaveLength(0);
    });
